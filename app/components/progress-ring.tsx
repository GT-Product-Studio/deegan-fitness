  ...
  return (
    <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
      <svg width={size} height={size} className="-rotate-90" style={{ position: "absolute" }}>
        ...
        <circle cx={size / 2} cy={size / 2} r={radius} fill="none" stroke="#00D26A" strokeWidth={strokeWidth} strokeLinecap="round"
          strokeDasharray={circumference} strokeDashoffset={offset} style={{ transition: "stroke-dashoffset 0.8s ease" }} />
      </svg>
      <div className="flex flex-col items-center justify-center z-10">
        <span className="text-3xl font-bold leading-none text-white">{Math.round(percent)}%</span>
        {label && <span className="text-sm font-semibold text-white mt-0.5">{label}</span>}
